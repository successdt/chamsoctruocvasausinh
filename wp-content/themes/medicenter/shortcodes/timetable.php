...
function get_timetable($departments_page, $department = null, $mode = null, $hour_category = null)
{
	...
	for($i=(int)$hours->min; $i<$max_hour; $i++)
	{
		...
		$output .= '<tr class="row_' . ($l+1) . ($l%2==0 ? ' row_gray' : '') . '">
						...
						{
							if(!in_array($j, (array)(isset($drop_columns[$i]["columns"]) ? $drop_columns[$i]["columns"] : array())))
							{
								if(hour_in_array($i, (isset($department_hours_tt[$j]) ? $department_hours_tt[$j] : array())))
								{
									$rowspan = get_rowspan_value($i, $department_hours_tt[$j], 1);
									if($rowspan>1)
									{
										for($k=1; $k<$rowspan; $k++)
											$drop_columns[$i+$k]["columns"][] = $j;	
									}
									$array_count = count($department_hours_tt[$j]);
									$hours = array();
									...
									{
										if(in_array((int)$department_hours_tt[$j][$k]["start"], $hours))
										{
											$departments[$department_hours_tt[$j][$k]["title"]]["name"] = $department_hours_tt[$j][$k]["name"];
											$departments[$department_hours_tt[$j][$k]["title"]]["tooltip"][] = $department_hours_tt[$j][$k]["tooltip"];
											$departments[$department_hours_tt[$j][$k]["title"]]["before_hour_text"][] = $department_hours_tt[$j][$k]["before_hour_text"];
											$departments[$department_hours_tt[$j][$k]["title"]]["after_hour_text"][] = $department_hours_tt[$j][$k]["after_hour_text"];
											$departments[$department_hours_tt[$j][$k]["title"]]["doctors"][] = $department_hours_tt[$j][$k]["doctors"];
											$departments[$department_hours_tt[$j][$k]["title"]]["id"] = $department_hours_tt[$j][$k]["id"];
											$departments[$department_hours_tt[$j][$k]["title"]]["hours"][] = $department_hours_tt[$j][$k]["start"] . " - " . $department_hours_tt[$j][$k]["end"];
											$department_hours_tt[$j][$k]["displayed"] = true;
										}
									}
									$color = "";
									$text_color = "";
