<?php
	...
	$image       = thb_get_featured_image($post_id, "medium");
	...
	if($i == $blog_posts->post_count-1) $post_classes[] = "last";
?>
	...
	<div class="item-wrapper <?php if( empty($image) && $post_format != "gallery" ) : ?>no-thumb<?php endif; ?>">
		...
		<div class="text">
			<?php
				...
				$excerpt = thb_truncate($excerpt, 250);
				echo apply_filters('the_content', $excerpt);
			?>
		</div>

		<a href="<?php the_permalink(); ?>" class="readmore btn small" rel="permalink"><?php _e(thb_get_option("_label_read", false, "Read more"), THEMENAME); ?></a>
